...
THISFILE = 'create-sqlite-tables-devt.py'
...
def dropTables(logger,cursor):
    ...
    cursor.execute('''DROP TABLE IF EXISTS attorneyTEMP''')
...
def createParty(logger,cursor): 
	cursor.execute('''
	             CREATE TABLE party (
	                 id integer UNIQUE PRIMARY KEY AUTOINCREMENT 
	               , partycounter integer
	               , partyname text
	               , partytype text
	               , partyterminated text
                       , fullcase_text text
	               , fullcase_id integer 
...
def createAttorneyTEMP(logger,cursor): 
	cursor.execute('''
	             CREATE TABLE attorneyTEMP (
	                 id integer UNIQUE PRIMARY KEY AUTOINCREMENT 
	               , partycounter integer
	               , attorneycounter integer
	               , attorneyname text
	               , attorneystatus text
                       , firmname text
                       , firmaddress text
...
dbfile = '/data2/dockets/sqlite/pydockets-devt.db'
try:
     os.remove(dbfile)
except:
     pass
...
print "about to do party"
createParty(mylogger,c)
print "about to do attorney"
createAttorneyTEMP(mylogger,c)
print "done creating"
